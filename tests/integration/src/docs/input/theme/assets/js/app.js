(function ($) {
  ...
  $(document).ready(function () {
    var commitHash = $(document.body).attr("commit-hash");
    $.get("/env.json?version=" + commitHash, function (data) {
      var url = data["searchBaseUrl"] + $(".search-form").attr("action");
  ...
  $(".search-form").submit(function (event) {
    ...
    $.get(url + "?text=" + values["text"], function (data) {
      $.each(data, function (index) {
        $(".search-results").append('<a href="' + data[index]["url"] +
          '" class="list-group-item list-group-item-action"><h5>' + data[index]["title"] +
          ' (' + data[index]["url"] + ')</h5><p>' + data[index]["contents"].join("<br/>") + '</p></a>');
      });
